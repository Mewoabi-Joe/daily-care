...
const MyLabTestCard = ({ id, name, image, price, state, bookedOn, doneOn, resultsOutOn }) => {
	console.log(state);
				...
				<div className="col-8">
					<div className="card-body p-0 ps-3">
						...
						<div className=" mt-sm-1 mt-md-2 mt-lg-3  mt-xl-5">
							<small className="text-muted ">Booked on</small> <small>{new Date(bookedOn).toDateString()}</small>
						</div>
						{state !== "PENDING" && (
							<div className=" mt-sm-1 mt-md-2 mt-lg-3  mt-xl-5">
						...
						{state === "RESULTS_OUT" && (
							<div className=" mt-sm-1 mt-md-2 mt-lg-3  mt-xl-5">
								<small className="text-muted ">Date when result was available</small>{" "}
						...
						<div className="d-flex justify-content-between mt-sm-1 mt-md-2 mt-lg-3 mt-xl-5">
							<button
								...
								onClick={() => setModal(<UpdateStateModal id={id} state={state} />)}
